...
def load_low():
    ...
    c = torch.load(str(Path(__file__).resolve().parents[1] / 'weights' / 'low_speed_6class.pth'),map_location=DEVICE,weights_only=False)
...
def load_high_single():
    ...
    c = torch.load(str(Path(__file__).resolve().parents[1] / 'weights' / 'high_speed_4class.pth'),map_location=DEVICE,weights_only=False)
    m.load_state_dict(c['model_state_dict']); return m
...
def load_high_multi():
    ...
    mc = torch.load(str(Path(__file__).resolve().parents[1] / 'weights' / 'high_multihead_K5.pth'),map_location=DEVICE,weights_only=False)
