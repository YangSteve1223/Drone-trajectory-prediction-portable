#!/usr/bin/env python3
"""
Global LoRA — general long-trajectory adaptation without overfitting.

Strategy:
  1. Start from ORIGINAL model (low_speed_6class.pth, NOT fine-tuned)
  2. Train LoRA on mixed windows from ALL trajectory lengths (short+medium+long)
  3. Low-rank bottleneck (~80K params vs 1.3M full model) prevents memorization
  4. Result: base model that handles all lengths well + generalizes

This produces a "general improvement LoRA" that can be stacked with per-drone LoRA.
"""
...
DT = 0.2
TOTAL_WINDOWS = 30000          # train on 30K mixed windows
TRAIN_SPLIT = 0.85             # 85/15 train/test
BATCH_SIZE = 64
SHORT_MAX = 80                 # frames
MEDIUM_MAX = 150
...
# ── Training ───────────────────────────────────────────────────────────────
EPOCHS = 30                    # fewer epochs for global training (more data)
LR_MAX, LR_MIN = 1e-3, 1e-5
WEIGHT_DECAY = 1e-4
GRAD_CLIP = 1.0
RESTARTS = 2                   # fewer restarts (global training is slower)
...
def make_windows(traj, stride=1):
    n = traj.shape[0]; ml = HIST_LEN*stride + PRED_LEN
    if n < ml: return [], []
    hists, futs = [], []
...
def main():
    print('='*80)
    print('Global LoRA Training — General long-trajectory adaptation')
    print(f'  Base: ORIGINAL model (low_speed_6class.pth)')
    print(f'  Data: {TOTAL_WINDOWS} mixed windows (short+medium+long)')
    print(f'  Epochs: {EPOCHS}  Restarts: {RESTARTS}')
    print('='*80)

    # Load ORIGINAL model (force, no fine-tuned)
    ...
    ckpt = torch.load(WEIGHT_DIR/'low_speed_6class.pth', map_location=DEVICE)
    model.load_state_dict(ckpt['model_state_dict'])
    print(f'  Original model loaded. Params: {sum(p.numel() for p in model.parameters()):,}')

    # ── Collect mixed data ──
    print(f'\n[2/4] Collecting {TOTAL_WINDOWS} mixed windows...')
    all_trajs = []
    for f in sorted(TRAJ_DIR.glob('*.npz')):
        d = np.load(f); n = d['traj'].shape[0]
        all_trajs.append((f.name, d['traj'], n))

    # Sample equally from 3 length groups
    short = [(n, t) for n, t, l in all_trajs if l < SHORT_MAX]
    medium = [(n, t) for n, t, l in all_trajs if SHORT_MAX <= l < MEDIUM_MAX]
    long_t = [(n, t) for n, t, l in all_trajs if l >= MEDIUM_MAX]
    print(f'  Short(<{SHORT_MAX}fr): {len(short)}  Medium({SHORT_MAX}-{MEDIUM_MAX}fr): {len(medium)}  Long(>={MEDIUM_MAX}fr): {len(long_t)}')
    ...
    for group_name, group in [('short', short), ('medium', medium), ('long', long_t)]:
        ...
        for name, traj in shuffled:
            ...
            hists, futs = make_windows(traj, stride=1)
    ...
    tr_data = all_windows[:n_tr]
    te_data = all_windows[n_tr:]
    print(f'  Train: {len(tr_data)}  Test: {len(te_data)}')

    # ── Base model evaluation (original, no LoRA) ──
    print(f'\n[3/4] Base model evaluation (original, no LoRA)...')
    base_preds = []
    for b in range(0, len(te_data), BATCH_SIZE):
        be = min(b+BATCH_SIZE, len(te_data))
        hb = torch.stack([d[0] for d in te_data[b:be]]).to(DEVICE)
        with torch.no_grad():
            base_preds.append(model(hb, force_predict=True)['predictions'].cpu())
    base_preds = torch.cat(base_preds, dim=0)
    te_t = torch.stack([d[1] for d in te_data])
    te_h = torch.stack([d[0] for d in te_data])

    base_fde = torch.norm(base_preds[:,-1,:]-te_t[:,-1,:], dim=-1).mean().item()
    base_ade = torch.norm(base_preds-te_t, dim=-1).mean(dim=1).mean().item()
    bdir = np.array([dir_err(base_preds[i,-1,:2].numpy(), te_data[i][1][-1,:2].numpy())
                     for i in range(len(te_data))])
    base_dir = float(np.mean(bdir))
    base_cata = float(np.sum(bdir >= 90) / len(bdir) * 100)
    base_gap = float(np.linalg.norm(base_preds[:,0,:].numpy(), axis=1).mean())
    print(f'  Original model: ADE={base_ade:.3f}m  FDE={base_fde:.3f}m  '
          f'Dir={base_dir:.1f} deg  Cata={base_cata:.1f}%  Gap={base_gap:.3f}m')

    # ── Global LoRA training ──
    print(f'\n[4/4] Training Global LoRA...')
    best_test_fde = float('inf'); best_state = None
    ...
    for restart in range(RESTARTS):
        ...
        tr_shuf = list(tr_data); np.random.shuffle(tr_shuf)

        for ep in range(EPOCHS):
            model.eval(); ep_losses = []
            for b in range(0, len(tr_shuf), BATCH_SIZE):
                be = min(b+BATCH_SIZE, len(tr_shuf))
                hb = torch.stack([d[0] for d in tr_shuf[b:be]]).to(DEVICE)
                tb = torch.stack([d[1] for d in tr_shuf[b:be]]).to(DEVICE)
                # Get base prediction for anchor loss
                with torch.no_grad():
                    bp = model(hb, force_predict=True)['predictions']
                opt.zero_grad()
                pred = model(hb, force_predict=True)['predictions']
                loss = compute_loss(pred, tb, hb, bp.detach(), ep)
                if not torch.isnan(loss) and not torch.isinf(loss):
                    loss.backward(); torch.nn.utils.clip_grad_norm_(params, GRAD_CLIP); opt.step()
                    ep_losses.append(loss.item())
            sched.step()
            if ep % 10 == 0:
                print(f'  Restart {restart+1} Epoch {ep:2d}: loss={np.mean(ep_losses):.4f}  lr={sched.get_last_lr()[0]:.6f}')
    ...
    ldir = np.array([dir_err(lora_preds[i,-1,:2].numpy(), te_data[i][1][-1,:2].numpy())
                     for i in range(len(te_data))])
    lora_dir = float(np.mean(ldir))
    lora_cata = float(np.sum(ldir >= 90)/len(ldir)*100)
    lora_gap = float(np.linalg.norm(lora_preds[:,0,:].numpy(), axis=1).mean())

    # Per-length-group breakdown
    print(f'\n{"="*80}')
    print(f'GLOBAL LoRA RESULTS')
    print(f'  Trainable params: {n_params:,} ({n_params/sum(p.numel() for p in model.parameters())*100:.1f}% of full model)')
    print(f'{"="*80}')
    print(f'  {"Metric":<22} {"Original":<14} {"+Global LoRA":<16} {"Change":<10}')
    print(f'  {"-"*62}')
    print(f'  {"ADE":<22} {base_ade:<14.3f}m {lora_ade:<16.3f}m {(base_ade-lora_ade)/max(base_ade,1e-6)*100:>+8.1f}%')
    print(f'  {"FDE":<22} {base_fde:<14.3f}m {lora_fde:<16.3f}m {(base_fde-lora_fde)/max(base_fde,1e-6)*100:>+8.1f}%')
    print(f'  {"Direction":<22} {base_dir:<14.1f} deg {lora_dir:<16.1f} deg {(base_dir-lora_dir)/max(base_dir,0.1)*100:>+8.1f}%')
    print(f'  {"Cata (>90deg)":<22} {base_cata:<14.1f}% {lora_cata:<16.1f}%')
    print(f'  {"Boundary Gap":<22} {base_gap:<14.3f}m {lora_gap:<16.3f}m')

    # Save global LoRA weights
    save_path = WEIGHT_DIR / 'global_lora.pth'
    torch.save({'lora_state': best_state['lora'], 'head_state': best_state['head'],
                'config': {'targets': LORA_TARGETS, 'heads': HEAD_TARGETS},
                'original_fde': base_fde, 'lora_fde': lora_fde},
               save_path)
    print(f'\n  Global LoRA saved: {save_path}')

    # Save JSON
    summary = {
        'original_model': 'low_speed_6class.pth',
        'global_lora': str(save_path),
        'train_windows': len(tr_data), 'test_windows': len(te_data),
        'trainable_params': n_params,
        'base_ade': base_ade, 'lora_ade': lora_ade,
        'base_fde': base_fde, 'lora_fde': lora_fde,
        'base_dir': base_dir, 'lora_dir': lora_dir,
        'base_cata': base_cata, 'lora_cata': lora_cata,
        'base_gap': base_gap, 'lora_gap': lora_gap,
        'fde_gain_pct': float((base_fde-lora_fde)/max(base_fde,1e-6)*100),
        'dir_gain_pct': float((base_dir-lora_dir)/max(base_dir,0.1)*100),
    }
    json.dump(summary, open(OUT_DIR/'global_lora.json','w'), indent=2)
    print(f'  Results saved: pic-results/global_lora.json')
    print('='*80)
