#!/usr/bin/env python3
"""
B3 — Direction-focused global LoRA on the 40-FRAME base.

Goal: reduce mean direction error (LOW ~17deg) without wrecking FDE.

Two levers vs train_lora_global.py:
  1. Rebalanced loss: W_DIR up (0.25->0.60), curvature/TV halved — let the
     direction term lead instead of being drowned by smoothness penalties.
  2. Turn-sample weighting: windows whose GROUND-TRUTH path turns more get a
     higher loss weight (up to 3x), so turns aren't averaged away by the
     straight-cruise majority.

Produces weights/dir_lora_40.pth. Compared 3-way (base / global_lora_40 / dir_lora_40).
"""
...
# ── Loss (B3: direction-forward rebalance) ─────────────────────────────────
BETA_HUBER = 0.20
W_DIR, W_SMOOTH, W_JERK = 0.60, 0.25, 0.25      # W_DIR up, smoothness down
W_CURVATURE, W_TV_VEL, W_SPEED = 0.10, 0.08, 0.03   # curvature/TV halved
W_ANCHOR_DIR = 0.02
W_BOUNDARY, BOUNDARY_STEPS = 0.40, 1
PHYSICS_WARMUP, PHYSICS_START = 3, 0.50

# Turn-sample weighting: weight = 1 + TURN_GAIN * clip((turn_deg-TURN_LO)/(TURN_HI-TURN_LO),0,1)
TURN_GAIN, TURN_LO, TURN_HI = 2.0, 20.0, 90.0
...
def turn_weight(fut):
    """Loss weight from ground-truth turn magnitude: angle between the first and
    last displacement direction of the future window (degrees)."""
    v0 = fut[1, :2] - fut[0, :2]
    v1 = fut[-1, :2] - fut[-2, :2]
    ...
    if n0 < 1e-3 or n1 < 1e-3:
        ...
        turn_deg = float(np.degrees(np.arccos(cos)))
    frac = min(max((turn_deg - TURN_LO) / (TURN_HI - TURN_LO), 0.0), 1.0)
    return 1.0 + TURN_GAIN * frac
...
def compute_loss(pred, target, history, base_pred, epoch, sample_w=None):
    ramp = physics_multiplier(epoch)
    # Per-sample turn weight applied to the two supervision terms (huber + dir).
    if sample_w is None:
        loss_huber = F.smooth_l1_loss(pred, target, beta=BETA_HUBER)
    else:
        w = sample_w.view(-1, 1, 1)
        huber_el = F.smooth_l1_loss(pred, target, beta=BETA_HUBER, reduction='none')
        loss_huber = (huber_el * w).sum() / (w.expand_as(huber_el).sum() + 1e-8)
    pred_vel = pred[:, 1:, :] - pred[:, :-1, :]; true_vel = target[:, 1:, :] - target[:, :-1, :]
    dir_el = (1.0 - F.cosine_similarity(pred_vel, true_vel, dim=-1))   # (B, P-1)
    if sample_w is None:
        loss_dir = dir_el.mean()
    else:
...
def main():
    print('=' * 80)
    print('Direction-focused LoRA Training — 40-FRAME base (B3)')
    ...
    for group_name, group in [('short', short), ('medium', medium), ('long', long_t)]:
        ...
        for name, traj in shuffled:
            ...
            for h, f in zip(hists, futs):
                w = turn_weight(f)
                all_windows.append((torch.from_numpy(h).float(), torch.from_numpy(f).float(), w))
                n_collected += 1
                if n_collected >= per_group: break
    ...
    wt_all = np.array([w for _, _, w in all_windows])
    print(f'  Total: {len(all_windows)}  Train: {len(tr_data)}  Test: {len(te_data)}')
    print(f'  Turn weights: mean={wt_all.mean():.2f}  max={wt_all.max():.2f}  '
          f'>{1.5:.1f}: {int((wt_all>1.5).sum())} windows')
    ...
    for restart in range(RESTARTS):
        ...
        for ep in range(EPOCHS):
            ...
            for b in range(0, len(idx), BATCH_SIZE):
                ...
                sw = tr_w[bi].to(DEVICE)
                opt.zero_grad()
                pred = model(hb, force_predict=True)['predictions']
                loss = compute_loss(pred, tb, hb, bp, ep, sample_w=sw)
    ...
    print(f'\n{"=" * 80}')
    print(f'DIRECTION LoRA (40-frame base) RESULTS')
    print(f'  Trainable params: {n_params:,} '
          f'({n_params / sum(p.numel() for p in model.parameters()) * 100:.1f}% of full model)')
    print(f'{"=" * 80}')
    print(f'  {"Metric":<18} {"40f base":<14} {"+Dir LoRA":<16} {"Change":<10}')
    print(f'  {"-" * 58}')
    ...
    save_path = WEIGHT_DIR / 'dir_lora_40.pth'
    ...
    json.dump(summary, open(OUT_DIR / 'dir_lora_40.json', 'w'), indent=2)
    print(f'  Results saved: pic-results/dir_lora_40.json')
    print('=' * 80)
